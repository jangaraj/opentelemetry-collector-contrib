...
func Test_exporter_start_InvalidClientConfig(t *testing.T) {
	...
	exp := newMetricsExporter(exportertest.NewNopSettings(metadata.Type), cfg)
...
func Test_LineTooLong(t *testing.T) {
	...
	intGaugeDataPoint.SetIntValue(10)
	intGaugeDataPoint.SetTimestamp(testTimestamp)
	exp := newMetricsExporter(exportertest.NewNopSettings(metadata.Type), &config.Config{DefaultDimensions: dims})
