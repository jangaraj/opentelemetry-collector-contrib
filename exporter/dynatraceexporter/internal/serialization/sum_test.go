...
func Test_serializeSumPoint(t *testing.T) {
	t.Run("without timestamp", func(t *testing.T) {
		...
		got, err := serializeSumPoint("int_sum", "prefix", dimensions.NewNormalizedDimensionList(), pmetric.AggregationTemporalityDelta, dp, ttlmap.New(1, 1, make(chan struct{}, 1)))
	...
	t.Run("int delta with prefix and dimension", func(t *testing.T) {
		...
		got, err := serializeSumPoint("int_sum", "prefix", dimensions.NewNormalizedDimensionList(dimensions.NewDimension("key", "value")), pmetric.AggregationTemporalityDelta, dp, ttlmap.New(1, 1, make(chan struct{}, 1)))
		assert.NoError(t, err)
...
func Test_convertTotalCounterToDelta_notMutating(t *testing.T) {
	...
	dp.CopyTo(orig)
	_, err := convertTotalCounterToDelta("m", "prefix", dimensions.NormalizedDimensionList{}, dp, ttlmap.New(1, 1, make(chan struct{}, 1)))
	assert.NoError(t, err)
