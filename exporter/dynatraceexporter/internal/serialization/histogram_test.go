...
func Test_serializeHistogramPoint(t *testing.T) {
	...
	hist.SetCount(2)
	hist.SetSum(9.5)
	hist.SetTimestamp(pcommon.Timestamp(time.Date(2021, 0o7, 16, 12, 30, 0, 0, time.UTC).UnixNano()))
	...
	t.Run("delta with non-empty first and last bucket", func(t *testing.T) {
		...
		histWithNonEmptyFirstLast.SetTimestamp(pcommon.Timestamp(time.Date(2021, 0o7, 16, 12, 30, 0, 0, time.UTC).UnixNano()))
	...
	t.Run("when average > highest boundary, max = average", func(t *testing.T) {
		...
		histWitMaxGreaterAvg.SetTimestamp(pcommon.Timestamp(time.Date(2021, 0o7, 16, 12, 30, 0, 0, time.UTC).UnixNano()))
	...
	t.Run("when average < lowest boundary, min = average", func(t *testing.T) {
		...
		histWitMinLessAvg.SetTimestamp(pcommon.Timestamp(time.Date(2021, 0o7, 16, 12, 30, 0, 0, time.UTC).UnixNano()))
	...
	t.Run("when min is provided it should be used", func(t *testing.T) {
		...
		minMaxHist.SetMin(3)
		minMaxHist.SetTimestamp(pcommon.Timestamp(time.Date(2021, 0o7, 16, 12, 30, 0, 0, time.UTC).UnixNano()))
